#!/bin/bash
# ===============================
# Samba DC Local Backup + Remote SCP Transfer
# With SSH Login Test and Remote Write Validation
# ===============================
...
FQDN=$(hostname -f)
DATE=$(date +%Y%m%d-%H%M)
BACKUP_ROOT="/root/samba-dc-backup"
TEMP_PIPE=$(mktemp -u)
mkfifo "$TEMP_PIPE"
...
"Enter local backup directory path:" 10 60 "$BACKUP_ROOT" 3>&1 1>&2 2>&3) || exit 0
mkdir -p "$BACKUP_DIR"
